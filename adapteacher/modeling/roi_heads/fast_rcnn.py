...
class FgFastRCNNOutputLayers(FastRCNNOutputLayers):
    def losses(self, predictions, proposals, branch, attack_mask=None):
        ...
        gt_scores = (
            cat([p.gt_scores for p in proposals], dim=0)
            if len(proposals)
            else torch.empty(0)
        ) if "gt_scores" in proposals[0]._fields else None
        ...
        if branch == "attack":
            ...
            if gt_scores is not None:
                ...
                loss_cls = torch.sum(-torch.log_softmax(scores, dim=1) * gt_scores, dim=1)
