...
class TATeacherTrainer(ATeacherTrainer):
    ...
    def generate_adversarial_pseudo_labels(self, pseudo_labels, attacked_pseudo_labels):
        ...
        for i in range(len(pseudo_labels)):
            ...
            # pseudo_labels[i].gt_weights = torch.ones_like(pseudo_labels[i].gt_classes, dtype=torch.float)
            ...
            # If pseudo labels or attacked predictions are empty, adversarial pseudo labels are also empty
            if len(pseudo_labels[i]) == 0 or len(attacked_pseudo_labels[i]) == 0:
                new_proposal_inst_adversarial.gt_boxes = Boxes(torch.zeros(0, 4))
                new_proposal_inst_adversarial.gt_classes = torch.zeros(0, dtype=torch.long)
    ...
    def resize(self, data, data_copied):
        ...
        for i in range(bs):
            ...
            if data_copied[i]["instances"].has("gt_boxes"):
                data_copied[i]["instances"].gt_boxes.tensor *= ratio
                data_copied[i]["instances"].gt_boxes.tensor[:, 0] += x1
                data_copied[i]["instances"].gt_boxes.tensor[:, 2] += x1
                data_copied[i]["instances"].gt_boxes.tensor[:, 1] += y1
                data_copied[i]["instances"].gt_boxes.tensor[:, 3] += y1
                data_copied[i]["instances"].gt_boxes.tensor = data_copied[i][
                    "instances"
                ].gt_boxes.tensor
