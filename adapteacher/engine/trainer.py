    ...
    def run_step_full_semisup(self):
        ...
        if self.iter < self.cfg.SEMISUPNET.BURN_UP_STEP:
            ...
            record_all_label_data, local_objectness, local_matrix = self.model(
                all_label_data, branch="supervised", ret_mean_objectness=True, ret_confusion_matrix=True
            )
            record_dict.update(record_all_label_data)
            ...
            for i in range(5):
                # print(i)
                unlabel_pertubation, _, _ = self.model(unlabel_data_q, branch="attack", class_info=self.imbalance_metric.roi[:,:-1], pertubation=pertubation)
