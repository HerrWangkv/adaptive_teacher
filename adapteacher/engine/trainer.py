...
class TATeacherTrainer(ATeacherTrainer):
    ...
    def run_step_full_semisup(self):
        ...
        if self.iter < self.cfg.SEMISUPNET.BURN_UP_STEP:
            ...
            pertubation = self.model_teacher(unlabel_data_q, branch="attack")
            pertubation *= -self.cfg.SEMISUPNET.ATTACK_SEVERITY / torch.tensor(self.cfg.MODEL.PIXEL_STD).to(pertubation.device).view(1,-1,1,1)

            #  5. set the labels back to pseudo labels generated by the teacher
            unlabel_data_q = self.remove_label(unlabel_data_q)
            unlabel_data_q = self.add_label(
                unlabel_data_q, pesudo_proposals_roih_unsup_k
            )
...
# Untargeted Attacked Teacher Trainer
class UATeacherTrainer(ATeacherTrainer):    
    # =====================================================
    # =================== Training Flow ===================
    # =====================================================
    ...
    def run_step_full_semisup(self):
        self._trainer.iter = self.iter
        assert self.model.training, "[UATeacherTrainer] model was changed to eval mode!"
        start = time.perf_counter()
        data = next(self._trainer._data_loader_iter)
        # data_q and data_k from different augmentations (q:strong, k:weak)
        # label_strong, label_weak, unlabed_strong, unlabled_weak
        ...
        if self.iter < self.cfg.SEMISUPNET.BURN_UP_STEP:
            ...
            #  2. generate the pseudo-label using teacher model
            with torch.no_grad():
                proposals_roih_unsup_k = self.model_teacher(unlabel_data_k, branch="unsup_data_weak")
            ...
            #  4. set the labels to pseudo labels generated by the teacher
            unlabel_data_q = self.remove_label(unlabel_data_q)
            unlabel_data_q = self.add_label(
                unlabel_data_q, pesudo_proposals_roih_unsup_k
            )
            ...
            #  5. strongly augmented unlabeled data
            # record_all_unlabel_data = self.model(
            ...
            #  6. conduct untargeted adversarial attack
            pertubation = self.model_teacher(unlabel_data_q, branch="attack")
            pertubation *= self.cfg.SEMISUPNET.ATTACK_SEVERITY / torch.tensor(self.cfg.MODEL.PIXEL_STD).to(pertubation.device).view(1,-1,1,1)

            #  7. adversarial training on strongly augmented unlabeled data
            ...
            #  8. input weakly labeled data (source) and weakly unlabeled data (target) to student model
            # give sign to the target data
            for i_index in range(len(unlabel_data_k)):
                for k, v in unlabel_data_k[i_index].items():
                    label_data_k[i_index][k + "_unlabeled"] = v
            ...
            for key in record_dict.keys():
                if key.startswith("loss"):
                    if key in ["loss_rpn_loc_pseudo", "loss_box_reg_pseudo", "loss_rpn_loc_attacked", "loss_box_reg_attacked"]:
                        loss_dict[key] = record_dict[key] * 0
                    # elif key[-6:] == "pseudo":  # unsupervised loss
                    #     loss_dict[key] = (
                    #         record_dict[key] * self.cfg.SEMISUPNET.UNSUP_LOSS_WEIGHT
                    #     )
                    elif key[-8:] == "attacked":  # unsupervised loss
                        loss_dict[key] = (
                            record_dict[key] * self.cfg.SEMISUPNET.UNSUP_LOSS_WEIGHT
                        )
                    elif ( # adversarial loss
    ...
    @torch.no_grad()
    def _update_aux_bn(self):
        student_model_dict = self.model.state_dict()
        for key in self.model.state_dict().keys():
            if "aux_bn" in key:
                student_model_dict[key] = student_model_dict[key.replace("aux_bn", "main_bn")]

        self.model.load_state_dict(student_model_dict)

# Domain Attacked Teacher Trainer

class DATeacherTrainer(ATeacherTrainer):
    ...
    def run_step_full_semisup(self):
        ...
        if self.iter < self.cfg.SEMISUPNET.BURN_UP_STEP:
            ...
            #  4. conduct targeted attack on all_label_data
            pertubation = self.model_teacher(all_label_data, branch="attack_domain")
            pertubation *= self.cfg.SEMISUPNET.ATTACK_SEVERITY / torch.tensor(self.cfg.MODEL.PIXEL_STD).to(pertubation.device).view(1,-1,1,1)
